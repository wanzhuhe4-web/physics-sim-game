...
你是一款名为《物理生存模拟》的文字 RPG 引擎。
语言风格幽默风趣，反映当代物理学研究生的生活。
...
def handle_action(action_text, input_type="ACTION", display_text=None):
    ...
    if input_type == "QUIZ_ANSWER":
        prompt = f"[ANSWER_QUIZ]: {action_text}。请评分，毒舌点评，然后继续主线剧情，必须给出 A/B/C 三个选项。"
        # 答完题后，AI 会给出评分和下一轮选项，所以模式回归 NORMAL
        # 注意：这里我们暂时设为 NORMAL，等拿到 AI 回复后再确认（通常不需要改）
        ...
        if is_quiz_round:
            ...
            prompt = f"{action_text} (系统指令：本轮是第 {st.session_state.round_count} 轮，是**强制考核回合**。描述完动作后果后，**绝对不要**给出 A/B/C 选项。请结合{field}领域出一道刁钻的简答题。)"
...
if not st.session_state.game_started:
    ...
    # Mode 2: Quiz (第 4 轮固定触发)
    elif st.session_state.mode == "QUIZ":
        st.warning("🚨 **考核时刻：导师的死亡凝视**")
        # 【核心修改】这里不再显示题目文本！
        # 题目已经作为 AI 的回复，显示在上面的聊天记录里了。
        # 这里只负责显示输入框。
        st.caption("请直接在下方输入框回答上方的物理问题。")
