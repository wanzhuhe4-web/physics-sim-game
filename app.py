...
# ⚡ 语言风格 (关键修改)
1. **极度精炼**：剧情描述必须控制在 **80 字以内** (3-4 句话)。不要写长篇大论。
2. **惜字如金**：像那个没时间看你论文的导师一样，说话短促、直接、毒舌。
3. **拒绝铺垫**：直接描述结果和后果，不要写心理活动。
...
2. **第 4 回合 (考核)**：
   ...
   - 直接出一道玩家相关专业的选择题 (不需要使用特殊标签，直接出题)。
3. **考核结算**：
...
def handle_action(action_text, input_type="ACTION", display_text=None):
    ...
    if input_type == "QUIZ_ANSWER":
        # 【修改点】优化了 Prompt，明确告诉 AI 玩家选择了哪个选项
        prompt = f"[ANSWER_QUIZ]: 我选择了选项 {action_text}。请核对正确答案，进行评分和毒舌点评，然后继续主线剧情，必须给出下一轮的 A/B/C 三个选项。"
        # 答完题后，AI 会给出评分和下一轮选项，所以模式回归 NORMAL
        ...
        if is_quiz_round:
            ...
            prompt = f"{action_text} (系统指令：本轮是第 {st.session_state.round_count} 轮，是**强制考核回合**。描述完动作后果后，**绝对不要**给出 A/B/C 选项。请结合{field}领域出一道简单的选择题。)"
            # 【关键】在这里直接锁定下一轮模式为 QUIZ
            next_mode_hint = "QUIZ"
        else:
    ...
    # 5. 处理 AI 回复 (完全不修改文字，原样展示)
    # 我们不再 split，也不再 strip，保持原汁原味
    ...
    # Boss 战检测 (仍需特殊处理模式)
        ...
        # 只有 Boss 战需要特殊分割，因为 UI 需要显示在不同地方
        # 如果你想纯粹一点，也可以不分割，直接显示在聊天框，然后下面给输入框
        # 这里为了兼容性保留简单处理
        
    else:
        # 常规/答题模式直接应用预判的模式
        st.session_state.mode = next_mode_hint
        if next_mode_hint == "QUIZ":
            st.toast("⚠️ 考核回合：导师突袭！", icon="🚨")

    # 6. 存入历史
    # 移除 plot data 标签以免影响阅读，其他不做修改
    clean_res = res.replace("[PLOT_DATA]", "").strip()
